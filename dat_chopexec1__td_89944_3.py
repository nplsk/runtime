...
sch = op('scheduleDAT')
log_dat = op('logDAT')  # Ensure a Table DAT named 'logDAT' exists in the network
...
def onValueChange(channel, sampleIndex, val, prev):
    ...
    log(f"onValueChange called: channel={channel.name}, val={val}, prev={prev}")
    log(f"Timeline value: val={val}, prev={prev}")
    ...
    log(f"  Parsed movement (int): {movement}")
    ...
    if current_move is None:
        log(f"current_move is None (movement={movement}); skipping scheduling.")
        return

    log(f"Movement idx: {movement}, Movement name: {current_move}")
    # DETECT WRAP: timeline CHOP value reset from max back to 0
    log("  Checking for timer wrap...")
    # Detect timer wrap (300→0) and reset row indices & advance movement
    if prev > 290 and val < 10:
        log(f"  Wrap detected! prev={prev}, val={val}")
        ...
        log(f"Movement channel: {mv_chan}")
        if mv_chan is not None:
            cur_mv = int(mv_chan)
            log(f"Current movement index: {cur_mv}")
            new_mv = cur_mv + 1
            if new_mv >= len(movement_names):
                ...
                log("End of performance: credits slide cue")
                return
            else:
                project.par.const6value = new_mv
                log(f"  Movement bumped to {new_mv}")
                log(f"New movement index: {new_mv}")
    ...
    log(f"  Layers to check for movement {movement}: {layers_to_check if 'layers_to_check' in locals() else 'TBD'}")

    # Iterate over each active layer to schedule its next clip
    # Iterate through each active layer and find its next clip
    for layer in layers_to_check:
        log(f"  Layer {layer}: starting idx lookup")
        # Get current rowIndex for this layer from the Constant CHOP
        try:
            p = project[f'rowIndex{layer}']  # CHOP channel
            idx = int(p)
            log(f"Param rowIndex{layer} = {idx}")
        except Exception as e:
            log(f"Error reading rowIndex{layer}: {e}")
            continue
        log(f"Checking layer {layer}, starting idx {idx}, current time {val}")
        log(f"  Searching schedule DAT for layer {layer}, movement {current_move}")
        ...
        while idx < sch.numRows:
            try:
                ...
                log(f"Row {idx}: move={row_move}, layer={row_layer}, time_start={row_t}")
            except:
                idx += 1
                continue
        ...
        if idx < sch.numRows:
            ...
            if val >= t:
                path = sch[idx, 3]
                log(f"Launching clip on layer {layer}: {path} at {val}s (scheduled {t}s)")
                log(f"    Advancing rowIndex{layer} to {idx+1}")
            ...
            log(f"No scheduled clip found for layer {layer}, movement {current_move}")
    return